...
global {
    ...
    //Subfactor lists (T1 - initial)
    list<float> subfactor_1_t1_list;
    list<float> subfactor_2_t1_list;
    list<float> subfactor_3_t1_list;
    list<float> subfactor_4_t1_list;
    list<float> subfactor_5_t1_list;
    
    //Subfactor lists (T2 - target)
    list<float> subfactor_1_t2_list;
    list<float> subfactor_2_t2_list;
    list<float> subfactor_3_t2_list;
    list<float> subfactor_4_t2_list;
    list<float> subfactor_5_t2_list;
    
    //Subfactor weights (parameters to vary)
    ...
    // helper list for easier computation
    list<float> weights <- [];
    ...
    init {
        ...
        file text_data <- text_file("/home/agropt/Gama_Workspace/models1/models/data/data_complete_anonymised.csv");
        ...
        // declare column indices for subfactors
        list<int> idx_sub_t1 <- list_with(5, -1);
        list<int> idx_sub_t2 <- list_with(5, -1);
        
        // find column indices for subfactors through loop
        loop i from: 1 to: 5 {
        	idx_sub_t1[i-1] <- headers index_of ("DBAttitudesT1.DB" + i + "."); // assign t1 indices to sub_t1
        	idx_sub_t2[i-1] <- headers index_of ("DBAttitudesT2.DB" + i + "."); // assign t1 indices to sub_t2
        	}
...
if idx_pro_reduction = -1 {
    ...
    return;
       ...
       ///if idx_id_group = -1 or idx_id = -1 or idx_db_t1 = -1 or idx_db_t2 = -1 or 
         ...
         //   write "ERROR: Required column not found!";
           // return;
        //}

	// Check subfactor indices
	loop i from: 0 to: 4 {
    	    if idx_sub_t1[i] = -1 or idx_sub_t2[i] = -1 {
        ...
        all_indices <- all_indices + idx_sub_t1 + idx_sub_t2;
        int max_idx <- max(all_indices);
        ...
        // filtering for t1 and t2 subfactors
        list<float> filtered_sub1_t1 <- [];
    	list<float> filtered_sub2_t1 <- [];
    	list<float> filtered_sub3_t1 <- [];
    	list<float> filtered_sub4_t1 <- [];
    	list<float> filtered_sub5_t1 <- [];
    
    	list<float> filtered_sub1_t2 <- [];
    	list<float> filtered_sub2_t2 <- [];
    	list<float> filtered_sub3_t2 <- [];
    	list<float> filtered_sub4_t2 <- [];
    	list<float> filtered_sub5_t2 <- [];
        ...
        loop idx over: valid_indices {
            ...
            // adding in new subfactors
            filtered_sub1_t1 << subfactor_1_t1_list[idx];
            filtered_sub2_t1 << subfactor_2_t1_list[idx];
            filtered_sub3_t1 << subfactor_3_t1_list[idx];
            filtered_sub4_t1 << subfactor_4_t1_list[idx];
            filtered_sub5_t1 << subfactor_5_t1_list[idx];
        
            filtered_sub1_t2 << subfactor_1_t2_list[idx];
        ...
        // replacement for new subfactor lists
        subfactor_1_t1_list <- filtered_sub1_t1;
    	subfactor_2_t1_list <- filtered_sub2_t1;
    	subfactor_3_t1_list <- filtered_sub3_t1;
    	subfactor_4_t1_list <- filtered_sub4_t1;
    	subfactor_5_t1_list <- filtered_sub5_t1;
    
    	subfactor_1_t2_list <- filtered_sub1_t2;
    	subfactor_2_t2_list <- filtered_sub2_t2;
    	subfactor_3_t2_list <- filtered_sub3_t2;
    	subfactor_4_t2_list <- filtered_sub4_t2;
    	subfactor_5_t2_list <- filtered_sub5_t2;
        ...
        loop i from: 0 to: length(debate_id_list) - 1 {
            if debate_id_list[i] = target_debate_id {
                create opinion_agent {
                    ...
                    // assign subfactors (T1)
                    subfactor_1_t1 <- subfactor_1_t1_list[i];
               	    subfactor_2_t1 <- subfactor_2_t1_list[i];
                    subfactor_3_t1 <- subfactor_3_t1_list[i];
                    subfactor_4_t1 <- subfactor_4_t1_list[i];
                    subfactor_5_t1 <- subfactor_5_t1_list[i];
                
                    // NEW: Assign target subfactors (T2)
                    subfactor_1_t2 <- subfactor_1_t2_list[i];
                    subfactor_2_t2 <- subfactor_2_t2_list[i];
                    subfactor_3_t2 <- subfactor_3_t2_list[i];
                    subfactor_4_t2 <- subfactor_4_t2_list[i];
                    subfactor_5_t2 <- subfactor_5_t2_list[i];
                    
                    // compute initial_opinion from weighted subfactors
                    list<float> current_subfactors <- [subfactor_1_t1, subfactor_2_t1, subfactor_3_t1,
		    ...
		    loop i from: 0 to: 4 {
		    	weighted_sum <- weighted_sum + weights[i] * current_subfactors[i];
		    	}
		    
                    // summation of weight of subfactor * subfactor / sum of all weights
                    initial_opinion <- weighted_sum / sum_weights;
                    
                    // setting up initial opinion
    ...
    action save_agent_results {
        ...
        loop ag over: opinion_agent {
            ...
            float mean_t2_subfactors <- (ag.subfactor_1_t2 + ag.subfactor_2_t2 +
            				  ag.subfactor_3_t2 + ag.subfactor_4_t2 + ag.subfactor_5_t2) / 5.0; 
	    
	    // calculation of individual subfactor errors (can use to compare)
	    float error_sub1 <- abs(ag.opinion - ag.subfactor_1_t2);
            ...
            save [
                ...
                ag.final_attitude,
                mean_t2_subfactors, // ****need to check this matches final_attitude
                
                // changes and errors
                opinion_change,
                individual_error, // error vs mean
                error_sub1, error_sub2, error_sub3, 
                error_sub4, error_sub5,  // Errors vs individual subfactors
...
species opinion_agent {
    ...
    //storing individual subfactors (T1)
    ...
    // storing individual subfactors (T2)
