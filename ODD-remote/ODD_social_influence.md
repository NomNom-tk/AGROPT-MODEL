...
**Undirected enforcement:**
After initial connection building:
    For each agent i:
        For each neighbor j in i.neighbors:
...
For each agent i with neighbors N(i):
    ...
    For each neighbor n in N(i):
        ...
        Else if diff ≥ i.agent_repulsion_threshold:
            // REPULSION ZONE
            direction = -1 if n.opinion > i.opinion else +1 // move away: push down if neighbor higher, push up if lower
            repulsion_force += direction  // Move away from dissimiar neighbor
            repulsive_count += 1
...
When speaking_mode is active, the neighbor set is replace by a single speaker for that cycle.
The update rules mirror 7.1.1-7.1.3 but with speaker_opinion substituting the neighbor loop.

For consensus_speaker:
    mean_opinion = mean([opinion_i, speaker_opinion])
    Δopinion = μ_i × (mean_opinion - opinion_i)

For clustering_speaker:
    If |speaker_opinion - opinion_i| ≤ ε_i:
        Δopinion = μ_i × (mean([opinion_i, speaker_opinion]) - opinion_i)
    Else: Δopinion = 0

For bipolarization_speaker:
    Same attraction/repulsion logic as 7.1.3
    but applied to single speaker_opinion instead of neighbor loop
...
**Fallback:** If max_cycles (300) reached without convergence, force stop

**Typical convergence:** 15-110 cycles for most parameter combinations (observed in initial testing)
...
**Current implementation: Complete network**
...
create_network():
    ...
    // Build complete connections within debate
    For each agent i (excluding control agents):
        For each agent j (same debate, not control, j ≠ i):
            i.neighbors.add(j)
    
    // Enforce undirected connections
    For each agent i:
        For each neighbor j in i.neighbors:
            If i not in j.neighbors:
                j.neighbors.add(i)

Note: Homophily-based probabilistic network was considered 
but replaced with complete network for simplicity and to 
avoid confounding network structure with opinion dynamics.
...
- Phase 3: LHS exploration (~300 samples per model)
