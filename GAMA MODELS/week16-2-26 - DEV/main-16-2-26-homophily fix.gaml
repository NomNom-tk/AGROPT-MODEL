...
global {
    ...
    action create_network {
        ...
        ask opinion_agent where (each.group_type != "Control") {
            loop potential_neighbor over: opinion_agent where (
                each != self and
        ...
        ask opinion_agent where (each.group_type != "Control" and length(each.neighbors) = 0) {
            opinion_agent closest <- opinion_agent with_min_of (
