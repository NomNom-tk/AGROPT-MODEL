...
## 3. NETWORK AND INTERACTION PARAMETERS (Parametric)

| Element ID | Name | Default Value | Plausible Range | Applicable Network | Notes |
|------------|------|---------------|-----------------|-------------------|-------|
| NET1 | network_type | "complete" | {complete} | Fixed at complete network | / |
| SP1 | speaking_mode | FALSE | {TRUE, FALSE} | When TRUE replaces standard neighbor interation with probabilistic turn-taking mechanic|
| SP2 | speak_weight | 1/(sum(recent_speech)+1) | Computed per agent | Inverse frequency weighting — agents who spoke less recently have higher probability of being selected |
| SP3 | recent_speech_window | N agents | Fixed | Rolling window size = number of agents in debate. Older entries dropped beyond window. |

**Network constraints:**
- Complete network: all agents within debate connected (current implementation)
- Speaking mode: mutually exclusive with standard neighbor interaction
- Control agents: always excluded from both network and speaking mechanic
- Networks are STATIC after initialization
- SP2 and SP3 only active when SP1 = TRUE
...
| Element ID | Name | Default Value | Plausible Range | Notes |
|------------|------|---------------|-----------------|-------|
| SIM1 | max_cycles | 300 | [50, 200] | Maximum simulation length (timesteps) |
