...
//  file opin_data <- csv_file("/home/agropt/AGROMOD/test_debates_30.csv", ",", true); // true = skip header
    ...
    reflex debug_every_cycle {
    	if mode_batch {
    		write "Batch cycle: " + cycle + "| Agents: " + length(opinion_agent);
    	}
    }
    ...
    reflex save_batch_results when: (cycle > max_cycles or fit_computed) and mode_batch {
        ...
        // progress writing
        write "Saving debate details. Number of debates: " + length(mae_per_debate.keys);
...
experiment social_influence type: gui {
    ...
    output {
        ...
        //debug mae check
    	monitor "DBG: debates tracked" value: mae_per_debate.keys;
    	monitor "DBG: MAE values" value: mae_per_debate;
    }
}
...
experiment Batch_clustering type: batch repeat: length(remove_duplicates(debate_id_list)) keep_seed: true until: end_simulation {
    ...
    init {
        ...
        write "=== Btach init complete";
        write "mode_batch: " + mode_batch;
