...
* - Removed unused old subfactor lists (replaced by subfactors_t1/t2)
*/
...
global {
    ...
    init {
        ...
        matrix data_matrix <- csv_file(text_data, separator =  ",", header = true);
        write "Total rows in file: " + data_matrix.rows;
        
        // Get column headers
        list<string> headers <- data_matrix.headers;

        // FALLBACK: If headers are missing (some GAMA versions don't parse them)
        int start_row <- 0;
        if length(headers) = 0 {
        ...
        loop idx from: 0 to: length(debate_id_list) - 1 {
            if debate_id_list[idx] = target_debate_id {
                create opinion_agent {
                    ...
                    // FIXED: Renamed loop variable from 'i' to 'j' to avoid shadowing
                    loop j from: 0 to: 4 {
                        weighted_sum <- weighted_sum + weights[j] * current_subfactors[j];
                    }
