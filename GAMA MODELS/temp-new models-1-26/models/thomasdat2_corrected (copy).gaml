...
global {
    ...
    init {
        ...
        text_data <- csv_file("../data/data_complete_anonymised.csv", ",");
        ...
        loop col from: 0 to: data_matrix.columns - 1 {
            headers << string(data_matrix[0, col]);
        }
        ...
        loop row_idx from: start_row to: data_matrix.rows - 1 {
            // Basic identifiers
            id_group_raw << string(data_matrix[row_idx, idx_id_group]);
            agent_id_list << int(data_matrix[row_idx, idx_id]);

            // Attitudes (normalized from [-6, +6] to [0, 1])
            float raw_t1 <- float(data_matrix[row_idx, idx_db_t1]);
            float raw_t2 <- float(data_matrix[row_idx, idx_db_t2]);
            initial_attitude_list << (raw_t1 + 6.0) / 12.0;
            final_attitude_list << (raw_t2 + 6.0) / 12.0;

            // Group type and pro/anti classification
            group_type_list << string(data_matrix[row_idx, idx_condition]);
            pro_reduction_list << int(data_matrix[row_idx, idx_pro_reduction]);

            // Parse subfactors (normalized from [-6, +6] to [0, 1])
            loop j from: 0 to: 4 {
                float t1_val <- (float(data_matrix[row_idx, idx_sub_t1[j]]) + 6.0) / 12.0;
                float t2_val <- (float(data_matrix[row_idx, idx_sub_t2[j]]) + 6.0) / 12.0;
        ...
        loop idx from: 0 to: length(debate_id_list) - 1 {
            if debate_id_list[idx] = target_debate_id {
                create opinion_agent {
                    ...
                    // FIXED: Renamed loop variable from 'i' to 'k' to avoid shadowing
                    loop k from: 0 to: 4 {
