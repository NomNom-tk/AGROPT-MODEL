...
reflex save_batch_results when: cycle = max_cycles and mode_batch {
        // Summary file
        save [model_type, convergence_rate, confidence_threshold,
              repulsion_threshold, repulsion_strength,
              seed, mae, opinion_variance, polarization_index]
        to: "batch_summary.csv" rewrite: false;
        ...
        loop debate_id over: mae_per_debate.keys {
            ...
            to: "batch_debate_details.csv" rewrite: false;
        }
    }    
    ...
    reflex compute_fit when: cycle = max_cycles {
		...
		loop debate over: unique_debates {
			...
			if length(debate_agents) > 0 {
				...
				mae_per_debate[debate] <- mean(debate_errors);
			}
		}
...
species opinion_agent {
    ...
    int final_attitude; // Store for comparison
     ...
     // batch parameter numeric
    int model_code;

    // global variable string to store model type
    string model_type;
...
// chose to try and create a batch exp per model due to previous errors when model varies models
experiment Batch_all_debates type: batch repeat: 1 keep_seed: true until: cycle >= max_cycles {
    ...
    /*// track previous sim for changes in write problems with save reflex
    global {
    	int prev_sim_index <- 0;
    }
    reflex save_results_intermediate when: mode_batch {  // triggers every cycle
     // trigger when simulation changes
     if self.index != prev_sim_index {
            ask simulations {
                save [
                    self.model_type,
                ...
                ] to: "results_all_debates_short.csv"
                  header: false  // only write headers once at first run
                  rewrite: false; // append results
            }
        }
    }  
    }

    // Save results final - ENHANCED to include per-debate MAE
    reflex save_results when: cycle = max_cycles {
        ask simulations {
            // Basic metrics
            save [
                self.model_type,
                ...
                self.polarization_index,
                self.mae_per_debate  // Per-debate breakdown
            ] to: "results_all_debates_final_.csv" header: true rewrite: false;
        }
    }
    */
}
