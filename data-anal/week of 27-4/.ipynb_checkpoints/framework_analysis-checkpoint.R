...
analyze_processed_run <- function(df) {
  ...
  if (!is.null(df_empirical)) {
    ...
    # ANALSCOPE: Hypotheses
    if (analysis_scope == "hypotheses") {
        # initialization of write_result for hypothese guard only 24/8/26
        ...
        # TODO Empirical calls (plain writing but no results) 24/8/26
    ...
    # Note: does not run with analysis_scope: sensitivity because df_ag is null and does not exist
  ...
  if (analysis_scope %in% c("hypotheses", "validation")) {
    ...
    if (!is.null(df_ag)) { # H1a and H1b implemented 22/7/26 / updated to robust model 24/7/26
      ...
      # Branch A: empirical trajectory (H1a) Sits in shared computations because validation block uses df_ag_base 8/9/26
      df_ag_base <- df_ag_raw_slice |>
        distinct(agent_id, debate_label, .keep_all = TRUE) # one real T1->T2 per agent/debate

      # ── HYPOTHESES ONLY: Branch A (H1a) and Branch B (H1b) ──────────────────
      if (analysis_scope == "hypotheses") {
        ...
        df_sim_base <- df_ag_raw_slice |>
          group_by(agent_id, debate_label, model_type, current_condition, design_cell, param_set_id) |>
          summarize(
            initial_opinion = first(initial_opinion),
            sd_opinion_seeds = sd(opinion, na.rm = TRUE),
            n_seeds = n_distinct(seed),
            opinion = mean(opinion, na.rm = TRUE),
            .groups = "drop"
          )
      ...
      if (!is.null(df$sim_val) && analysis_scope == "validation") {
        ...
        df_h3a <- df$sim_val$df_ag %>%
          ...
          left_join(df_empir_test %>% select(agent_id, debate_label, mlm_error),
                    by = c("debate_label", "agent_id")) %>%
      ...
      # ── SHARED: Branch C — MAE / OLS/ABM Comparison ─────────────────────────
      # In sample exploratory - calib and fitted on 43 debates
      ...
      if (!is.null(empir_cohen) && analysis_scope == "hypotheses") {
        ...
        simulated_betas_raw <- df_ag_deduped %>%
          ...
          do({
            ...
            if (nrow(dat_sim) >= 2 && has_var_y && has_var_x) {
              ...
              fit_sim <- lm(opinion_change_z ~ pro_reduction_z, data = dat_sim)
        ...
        if (nrow(simulated_betas_raw) > 0 && "std_estimate" %in% colnames(simulated_betas_raw)) {
          beta_distance <- simulated_betas_raw %>%
            group_by(model_type) %>%
            summarize(
              mean_simulated_beta = mean(std_estimate, na.rm = TRUE),
      ...
      } # end beta distance hypotheses guard

    } # end df_ag null guard

  } # end analysis_scope guard
  
  # ────────────────────────────────────────────────────────────────────────────
  # 4. BEHAVIORAL EXTRACTIONS (COMPOSITION & SPEAKING MODES)
  # ────────────────────────────────────────────────────────────────────────────
  conv_model_result <- NULL
  h_vs_m <- NULL
  speaking_compar <- NULL
  conv_diff <- NULL
  ...
  if (analysis_scope == "sensitivity") {  
      ...
      conv_model_result <- run_conv_model(df_conv_debate)
...
# TODO Request GLPK package for RcppParallel for Network analysis
  ...
  analysis_output_package <- list(
    inputs = list(
      ...
      analysis_scope   = config$analysis_scope # infers scope from config to retrieve objects based on running scope
    ),
    results = list(
      sensitivity = list(
      ...
      models = list(conv = conv_model_result, 
